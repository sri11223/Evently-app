...
export class DatabaseInitController {
    ...
    async initializeDatabase(req: Request, res: Response): Promise<void> {
        try {
            ...
            const schemaSql = `
                ...
                -- Waitlist promotions tracking table (FIX FOR MISSING TABLE ERROR)
                ...
                -- Add admin2@evently.com for testing (the working admin account)
                INSERT INTO users (email, name, password_hash, role) VALUES 
                ('admin2@evently.com', 'Admin User', '$2b$12$KIX8YhkOoXm4V7VJ7FoI0.p/8r.QF5KrZFp3k.BJ7FK.9FJe.EaBq', 'admin'),
            ...
            console.log('✅ Database schema created successfully (including waitlist_promotions fix)');
            ...
            res.status(200).json({
                success: true,
                message: 'Database initialized successfully with waitlist_promotions table fix',
                tables: tableNames,
                recordCounts: counts,
                timestamp: new Date().toISOString(),
                fix_applied: 'waitlist_promotions table added to prevent WaitlistManager errors'
            });
    ...
    async getDatabaseStatus(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Check if waitlist_promotions exists
            const waitlistPromotionsExists = tables.rows.some(row => row.table_name === 'waitlist_promotions');

            res.status(200).json({
                success: true,
                status: 'connected',
                database: process.env.NODE_ENV === 'production' ? 'PostgreSQL (Production)' : 'PostgreSQL (Development)',
                tables: tables.rows.map(row => ({
                    name: row.table_name,
                    columns: row.column_count,
                    records: counts[row.table_name]
                })),
                waitlist_promotions_exists: waitlistPromotionsExists,
                connection_pool: {
...
export default DatabaseInitController;